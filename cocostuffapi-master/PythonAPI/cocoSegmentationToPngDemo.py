...
def cocoSegmentationToPngDemo(dataDir='/home/lyu4/datasets/car_seg_coco/car_seg_dataset', dataTypeAnn='val2017', dataTypeRes='examples', \
    ...
    annPath = '%s/annotations/instances_%s.json' % (dataDir, dataTypeAnn)  #改
    if isAnnotation:
        # pngFolder = '%s/annotations/%s' % (dataDir, pngFolderName)
        pngFolder = '%s/stuffthingmaps/%s' % (dataDir, dataTypeAnn)  #改
    else:
